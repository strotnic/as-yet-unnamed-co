...
void portInit() {
  ...
  (DIGITAL_PIN_TYPE[10] == 0) ? pinMode(D10, OUTPUT) : pinMode(D10, INPUT_PULLUP);
  (DIGITAL_PIN_TYPE[11] == 0) ? pinMode(D11, OUTPUT) : pinMode(D11, INPUT_PULLUP);
  (DIGITAL_PIN_TYPE[12] == 0) ? pinMode(D12, OUTPUT) : pinMode(D12, INPUT_PULLUP);
